...
class _OpenAICompatClient:
    ...
    def _do_create(self, *, model: str, messages: list,
        ...
        if resp.status_code != 200:
            ...
            raise RuntimeError(
                f"{self._provider} API error {resp.status_code}: {err_body}"
            )
        data = resp.json()
...
class LLMKeyManager:
    ...
    def any_provider_available(self) -> bool:
        """True if ANY provider (Groq/Cerebras/SambaNova/OpenRouter/Gemini)
        has at least one available key.  Used by master_analyst to decide
        whether to even attempt an LLM call vs. going straight to the
